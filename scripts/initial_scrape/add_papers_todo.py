...
for search1 in searchList1:
	for search2 in searchList2:
		for search3 in searchList3:
			...
			sql0 = f"select * from searches where search_terms = '{searchID}'"
			cur.execute(sql0)
			sql0Rows = cur.fetchall()

			#if not in db yet then add
			#if (len(sql1Rows)+len(sql2Rows))==0:
			#if (len(sql0Rows))==0:
			if (True):
			#encode parameters
				...
				#target right column based on database
				dbcheck = "paper_id"
				database = "pubmed"
				...
				for resultID in results:
					allCount = allCount+1
					#check paper has not already been logged
					sql1 = f"SELECT * FROM paper where database_paper_id={resultID}"
					cur.execute(sql1)
					sql1Rows = cur.fetchall()

					#check paper is not already in to do list
					sql2 = f"SELECT * FROM todo where {dbcheck}={resultID}"
					cur.execute(sql2)
					...
					if (len(sql1Rows)+len(sql2Rows))==0:
						...
						newCount=newCount+1
						if db == 'pubmed':
							#print(f'https://www.ncbi.nlm.nih.gov/pubmed/{resultID}')
							z=1
						if test!=True:
							sql3 = f"INSERT INTO todo({dbcheck}, search_id, database) VALUES ({resultID}, '{searchID}', '{database}');"
							cur.execute(sql3)
							conn.commit()

				#give summary of new papers added
				print(f"{searchID}|{newCount}|{allCount}")
				batchID = test
				#####pick up here tmomrrow
				#####insert search and then loop and insert each number
				sql4 = f"INSERT INTO searches(search_terms, database, papers_add,papers_found) VALUES ('{searchID}', 'pubmed',{newCount},{allCount}) RETURNING search_id;"
				cur.execute(sql4)
				conn.commit()
				newSearchID = cur.fetchone()[0]
				for resultID in results:
					sql5 = f"INSERT INTO search_paper_ids(search_id, paper_id) VALUES ({newSearchID},{resultID});"
					cur.execute(sql5)
					conn.commit()
